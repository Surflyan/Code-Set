...
{   
	...
	finish1 = clock();
	timeb = (double)(finish1 - start1) / CLOCKS_PER_SEC;
	PrintSeq(Seq, NumDigit);
	...
	finish = clock();
	timeq =( double)(finish - start) / CLOCKS_PER_SEC;
	PrintSeq(Seq, NumDigit);

	
	CreatRandomDigit(Seq, NumDigit);
	cout << endl << "选择排序： " << endl;
	SelectSort(Seq, NumDigit);
	//PrintSeq(Seq, NumDigit);

	CreatRandomDigit(Seq, NumDigit);
	cout << endl << "堆排序： " << endl;
	HeapSort(Seq, NumDigit);
	//PrintSeq(Seq, NumDigit);

	CreatRandomDigit(Seq, NumDigit);
	cout << endl << "插入排序： " << endl;
	InsertSort(Seq, NumDigit);
	//PrintSeq(Seq, NumDigit);
